...
def runclaw(xclawcmd=None, outdir=None, overwrite=True, restart=None, 
            ...
            xclawout=None, xclawerr=None, verbose=True, dtopo=None):
    """
    ...
    if dtopo is not None: #for running batch from python by Naveen 25 Oct 2021 in setbatch.py
        outdir = os.path.join(outdir, dtopo.split('.t')[0])
    ...
    if datafiles == ():
        ...
        if rundir != outdir:
            ...
            if dtopo is not None: ##for running batch from python by Naveen 25 Oct 2021 in setbatch.py
                olfile = open(os.path.join(outdir,'dtopo.data'), 'r')
                updlines=''
                for line in olfile:
                    line = line.strip()
                    replace = line.replace('TEST.tt3', dtopo)
                    if dtopo[-5:] == 'txydz':
                        replace = line.replace('TEST.tt3', dtopo).replace('3   # dtopo_type','1   # dtopo_type')
                    updlines = updlines + replace+'\n'
                olfile.close()
                nufile=open(os.path.join(outdir,'dtopo.data'), 'w')
                nufile.write(updlines)
                nufile.close()
