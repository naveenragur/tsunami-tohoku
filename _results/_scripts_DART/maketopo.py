...
import math, pandas, os, numpy, glob, gc
...
def make_dtopo(EveMw,EveLat,EveLon,EveDep,makeplots=False):
    ...
    dtopo_fname = os.path.join(DtopoDir,'tohoku_dtopo_' + str(EveMw) + "_" + str(EveLat) + "_" + str(EveLon) + '.tt3')
    png_fname = os.path.join(PngDir,'tohoku_dtopo_' + str(EveMw) + "_" + str(EveLat) + "_" + str(EveLon) + '.png')
...
def make_hisdtopo(tohoku_subfaults,fname,makeplots=False):

    dtopo_fname = os.path.join(DtopoDir, fname[:-3] + 'tt3')
    png_fname = os.path.join(PngDir, fname[:-3] + 'png')
    asc_fname = os.path.join(ASCDir, fname[:-3] + 'asc')
    prj_fname = os.path.join(ASCDir, fname[:-3] + 'prj')
    ...
    if tohoku_subfaults.shape[1] < 13:
        ...
        fault = dtopotools.Fault(coordinate_specification='bottom center')
        fault.subfaults = []
        for i in range(len(tohoku_subfaults)):
            subfault = dtopotools.SubFault()
            subfault.length = tohoku_subfaults.L[i] * 1000
...
def get_topo(makeplots=False):
    ...
    if 1:
        print(os.getcwd())
        topo = topotools.Topography(path='/home/nrr/Tsunami/Tohoku/gis/topo/ASC/depth_wgs_gebco.asc', topo_type=2)
        shore = topo.make_shoreline_xy()
        plt.plot(shore[:, 0], shore[:, 1])
        plt.savefig('/home/nrr/Tsunami/Tohoku/gis/coastlines/gebco_coastline.png')
        numpy.save(shorelines_file, shore)

    if makeplots:
        topo_filepaths = glob.glob('/home/nrr/Tsunami/Tohoku/gis/topo/ASC/*wgs*.asc')
...
if __name__ == "__main__":
    ...
    DtopoDir = os.path.join(root_dir,'gis','dtopo')
    ...
    shorelines_file = '/home/nrr/Tsunami/Tohoku/gis/coastlines/JPshoreline_xy.npy'

############################ DTOPO FILES ################################
# -----------------------------------------------------------------------------------------------------------------------
    # 2011 historic event in Tohoku Region v2

    fnames = os.listdir(os.path.join(root_dir, 'gis', '2011','source','_input2script'))
    for fname in fnames:
        subfaultdf = pandas.read_csv(os.path.join(root_dir, 'gis', '2011','source','_input2script',fname))
        make_hisdtopo(subfaultdf, fname, True)

#-----------------------------------------------------------------------------------------------------------------------
    # # Sythetic Events in Tohoku Region
    # MW_VALS = [7.5, 8, 8.5, 9]
    # for EveMw in MW_VALS:
    #     for i in range(len(subfaultdf)):
    #         make_dtopo(EveMw, subfaultdf.LAT[i], subfaultdf.LON[i], subfaultdf.D[i], makeplots=True)
